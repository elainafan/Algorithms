...
普通线性基不能像前缀和一样相减，需要在每个基向量上额外维护位置：
    b[k][i]：前 k 个数中，最高位为 i 的基向量。
    pos[k][i]：这个基向量的一种表示中，所用原数组元素的最小下标加 1；空位为 0。
插入时，同一最高位优先保留下标更靠右的向量，较旧的向量继续向低位消元。
查询 [l, r] 时，b[r+1] 中满足 pos[r+1][i] >= l+1 的向量恰好构成该区间的线性基。
注意 pos 不是“最后一次插入的下标”，向量经过异或后可能由多个原数合成。

原数组 a 和查询 [l, r] 均为 0-based，查询使用闭区间；版本 k 表示前 k 个元素。
版本 0 为空，每次 insert 都追加一个元素，即使插入的是 0 或线性相关的数。
查询需满足 0 <= l <= r < 已插入元素个数；支持末尾追加，不支持直接修改历史元素。
...
用法：
    ...
    // [l, r] 为 0-based 闭区间，需满足 0 <= l <= r < 已插入元素个数，以上查询均允许空集。
